...
final class MinecraftSensor {
	...
	StoneAcquisition.World observe(MinecraftClient client, long tick, java.util.Set<Pos> retained) {
		...
		seen.forEach((pos, value) -> {
			...
			if (value.identified()) memory.put(pos, value);
		});
		...
		ObservedTerrain.publishObserved(memory);
	...
	void clear() { memory.clear(); footholds.clear(); previousPlayer = null; previousWorld = null; life = 0; ObservedTerrain.clear(); }
}
