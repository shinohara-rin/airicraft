...
The older, disconnected in-process verification DSL and its sole remaining test have been removed. The reusable evaluator is now the only in-game scenario framework in the tree, so a GameTest adoption would not need to coexist with or port that legacy runner.
